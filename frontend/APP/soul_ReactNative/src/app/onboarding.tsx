...
export default function OnboardingScreen() {
  ...
  return (
    <View style={styles.container}>
      {/* Tiêu đề ứng dụng - Gradient SOUL logo */}
      <LinearGradient
        ...
        style={styles.header}
      >
        <Text style={[styles.headerText, { color: "#FFFFFF", letterSpacing: 6 }]}>SOUL</Text>
      </LinearGradient>
        ...
        >
          {onboardingCards.map((card) => {
            ...
            return (
              ...
              >
                {isActive ? (
                  // Active card: gradient icon background
                  <LinearGradient
                  ...
                  <View style={[styles.iconContainer, styles.inactiveIconBg]}>
                    <MaterialCommunityIcons
                      name={card.icon as any}
                      size={22}
                      color="#8193A5"
      ...
      {/* 3. Dấu chấm hiển thị chỉ số trang (Indicators) - gradient active dot */}
      ...
      {/* 4. Thanh trượt "Swipe to start" với gradient handle */}
      <View style={styles.sliderTrack}>
        <Animated.Text style={[styles.sliderText, { opacity: textOpacity }]}>
          Swipe to start
        </Animated.Text>
        <Animated.View
          style={[styles.sliderHandle, { transform: [{ translateX: pan }] }]}
